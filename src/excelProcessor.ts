...
/**
 * Excel processor for NF-e (Nota Fiscal Eletrônica) normalization.
 * 
 * This module processes Excel files containing purchase and sale invoices,
 * normalizing the data for storage in Supabase:
 * - Purchase invoices (entrada/compra) → `purchase_invoices` table
 * - Sale invoices (saída/venda) → `sale_invoices` table
 * 
 * The n8n automation workflow handles routing data to the appropriate table
 * based on the detected invoice_type.
 */
...
export function processExcel(input: Buffer | Uint8Array | string): NormalizedOutput {
  let buf: Buffer;
...
export type { NormalizedOutput, NormalizedRecord };
